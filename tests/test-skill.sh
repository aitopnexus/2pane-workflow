...
grep -q 'Start each requested action directly with one standalone' "$SKILL"; check "inbox actions start directly with one helper call" $?
...
grep -q 'successful send may print nothing' "$SKILL"; check "empty successful send output is final" $?
grep -q 'Report once and yield' "$SKILL"; check "completed action yields to the human router" $?
grep -q 'Never retry, append probes, or inspect.*\.2pane/.*INBOX.md.*consuming.md' "$SKILL"; check "skill forbids retries, probes, and runtime inspection" $?
